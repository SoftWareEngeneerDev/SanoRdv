...
export const ajouterPatient = async (req, res) => {
  try {
    let {
      nom,
      prenom,
    ...
    if (!sex) champsManquants.push("sex");
    if (!dateNaissance) champsManquants.push("dateNaissance");
    ...
    const existe = await Patient.findOne({ email });
    if (existe) {
      return res.status(400).json({
    ...
    const genererIDPatient = async () => {
      const dernierPatient = await Patient.findOne(
        { IDpatient: { $regex: /^PAT-\d{4}$/ } },
        { IDpatient: 1 }
      ).sort({ IDpatient: -1 });

      let numero = 1;
      if (dernierPatient && dernierPatient.IDpatient) {
        const parts = dernierPatient.IDpatient.split('-');
        numero = parseInt(parts[1]) + 1;
      }

      return `PAT-${numero.toString().padStart(4, '0')}`;
    };

    const IDpatient = await genererIDPatient();

    // Générer avatar si pas de photo
    const photo = generateAvatarBase64(nom, prenom);
